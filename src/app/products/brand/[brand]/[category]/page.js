...
export async function generateStaticParams() {
  ...
  return products
    ...
    .map((p) => ({ brand: encodeURIComponent(p.brand), category: p.category }));
}
...
export default async function BrandCategoryPage({ params }) {
  ...
  const [products, allProducts] = await Promise.all([
    getBrandCategoryProducts(brand, category),
    getAllProducts(),
  ]);
      ...
      <main className="relative pt-28">
        <section className="mx-auto w-full max-w-6xl px-6 pb-24 pt-10 sm:px-10">
          ...
          <div className="mt-4 flex flex-col gap-3 animate-fade-up" style={{ animationDelay: "0ms" }}>
            <p className="text-xs font-semibold uppercase tracking-[0.4em] text-[#00004d]/70">{brand} {label}</p>
