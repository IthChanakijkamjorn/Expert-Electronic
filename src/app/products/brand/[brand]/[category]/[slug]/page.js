...
export async function generateStaticParams() {
  const products = await client.fetch(
    `*[_type == "product" && defined(slug.current) && defined(brand) && defined(category)] {
      brand, category, "slug": slug.current
    }`
  );
  return products
      ...
      <main className="relative pt-28">
        <section className="mx-auto w-full max-w-6xl px-6 pb-24 pt-10 sm:px-10">
          ...
          <nav className="flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.3em] text-[#00004d]/60">
            ...
            <span>/</span>
            <Link href={`/products/brand/${encodeURIComponent(brand)}/${category}`} className="hover:text-[#00004d]">{categoryLabel}</Link>
            <span>/</span>
